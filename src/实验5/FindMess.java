...
public class FindMess {
    public static void main(String args[]) {
        ...
        if (mess.contains("程序")) { // 【代码 1】判断 mess 中是否含有"程序"
            System.out.println("图书信息包含有\"程序\"");
        }

        int index = mess.indexOf(":", mess.indexOf(":") + 1); // 【代码 2】mess 调用 indexOf(String s,int start)返回 mess 中第 2 个冒号的位置
        String date = mess.substring(index + 1, index + 11);
        System.out.println(date);

        int pricePosition = mess.indexOf("价格"); // 【代码 3】mess 调用 indexOf(String s)返回首次出现"价格"的位置
        ...
        index = mess.lastIndexOf(":"); // 【代码 4】mess 调用 lastIndexOf(String s)返回最后一个冒号位置
