...
public class ShowFrame extends JFrame implements ActionListener {
    ...
    ShowFrame() {
        treeSet= new TreeSet<Program>(); showArea=new JTextArea(); showArea.setFont(new Font("",Font.BOLD,20));
        ...
        pNorth.add(button); add(pNorth,BorderLayout.NORTH); add(new JScrollPane(showArea),BorderLayout.CENTER); setSize(620,320); setVisible(true); setDefaultCloseOperation(DISPOSE_ON_CLOSE); validate();
    }
    public void actionPerformed(ActionEvent e) {
        ...
        treeSet.add(program); show(treeSet);
    }
    public void show(TreeSet tree) {
        showArea.setText(null);
        Iterator<Program> te= treeSet.iterator(); while(te.hasNext()) {
            Program pro= te.next();
            String pattern = "%tY-%<tm-%<td/%<tT";
            String strTime = String.format(pattern,pro.getLocalDateTime()); showArea.append(pro.getName()+"(演出时间):\n"+strTime+"\n");
        }
    }
}
