...
public class Number implements Runnable {
    ...
    Number() {
//创建 giveNumberThread,当前 Number 类的实例是 giveNumberThread 的目标对象
        giveNumberThread = new Thread(this);
        giveNumberThread.setName("giveNumberThread");
//创建 guessNumberThread,当前 Number 类的实例是 guessNumberThread 的目标对象
        guessNumberThread = new Thread(this);
        guessNumberThread.setName("guessNumberThread");
    }
    public void run() {
    ...
    public synchronized void setMessage(int count) {
        ...
        if(Thread.currentThread()==giveNumberThread) { while(pleaseGuess==true)
            ...
            else if(realNumber<guessNumber) { message=LARGER;
        ...
        if(Thread.currentThread()==guessNumberThread&&isGiveNumber==true) { while(pleaseGuess==false)
            try { wait(); //让出 CPU 使用权，让另一个线程给出提示
            }
            catch(InterruptedException e){}
            if(message==SMALLER) { min=guessNumber;
                guessNumber=(min+max)/2;
                System.out.println("我第"+count+"次猜这个数是:"+guessNumber);
            }
            else if(message==LARGER) { max=guessNumber;
                guessNumber=(min+max)/2;
                System.out.println("我第"+count+"次猜这个数是:"+guessNumber);
            }
            pleaseGuess=false;
