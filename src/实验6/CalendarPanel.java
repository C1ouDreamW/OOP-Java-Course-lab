...
public class CalendarPanel extends JPanel {
    ...
    public CalendarPanel() {
        calendar = new GiveCalendar(); currentDate = LocalDate.now();
        dataArrays = calendar.getCalendar(currentDate); showCalendar(dataArrays);
    }
    public void showCalendar(LocalDate [] dataArrays) {
        ...
        for(int i=0;i<dataArrays.length;i++){
            if(dataArrays[i] != null){
                showDay[i].setText(""+dataArrays[i].getDayOfMonth());
            }
        }
        repaint();
    }
    ...
    public int printSpace(DayOfWeek x) {
        int n = 0; switch(x) {
            case SUNDAY: n=0;
                break; case MONDAY: n=1;
                break; case TUESDAY: n=2;
                break; case WEDNESDAY:n=3;
                break; case THURSDAY: n=4;
                break; case FRIDAY: n =5;
                break; case SATURDAY: n = 6;
                break;
        }
        return n;
    }
